...
app = typer.Typer()
backend = backends.initialize(BACKEND)


@app.command()
def ingest(user_id: str, url: str):
    user = cmd.user.get_user(backend.users, user_id)
    cmd.content.ingest_content(user, backend.content, url, embedding_model=EMBED_MODEL)
...
def batch_insert(
    ...
    cmd.content.batch_insert_content(
        user, raw_content_repo, backend.content, sample=sample
    )


@app.command()
def get(content_id: str):
    content = cmd.content.get_content(backend.content, content_id)
    print(content.dict())
...
def search(query_text: str, limit: int = 10, provider: Optional[str] = None):
    for content in cmd.content.search_content(
        backend.content, query_text, limit=limit, provider=provider
    ):
...
def similar(content_id: str, limit: int = 10):
    for content in cmd.content.similar_content(
        backend.content, content_id, limit=limit
    ):
        print(content.id, content.title, content.url)
